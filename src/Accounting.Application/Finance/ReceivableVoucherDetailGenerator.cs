...
{
    public class ReceivableVoucherDetailGenerator : IDisposable
    {
        ...
        private GenerateReceivableDetailRequestDto Input { get; set; }
        IAbpLazyServiceProvider LazyServiceProvider { get; set; }

        public ReceivableVoucherDetailGenerator(
           ...
           )
        {
            ...
            { 
                Guid subjectId = await GetArSubjectIdAsync();
                //没有选收了哪张单的钱
                if (receipts == 0)
                {
        ...
        private async Task<Guid> GetArSubjectIdAsync()
        {
            var appSettingService = LazyServiceProvider.GetRequiredService<IAccountingSettingAppService>();
            var arSubjectIdText = await appSettingService.GetAccountReceivableSubjectCodeAsync();
            Guid subjectId;
            if (string.IsNullOrEmpty(arSubjectIdText) ||
                !Guid.TryParse(arSubjectIdText, out subjectId))
            {
                throw new BusinessException(VoucherErrorCodes.PleaseEnterArSubjectInSetting);
            }
            return subjectId;
        }
