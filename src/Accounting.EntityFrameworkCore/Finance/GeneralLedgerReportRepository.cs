...
namespace Accounting.Finance
{
    public class GeneralLedgerReportRepository : IGeneralLedgerReportRepository
    {
        ...
        {
            var queryable = _voucherQueryable;

            var lastYearBfQuery = queryable.Where(item => item.VoucherDate < periodStartDate)
            ...
            var currentYearBfQuery = queryable.Where(item => item.VoucherDate >= periodStartDate && item.VoucherDate < startDate)
                                            .SelectMany(item => item.Details)
                                            .WhereIf(subjectId != null, item => item.SubjectId == subjectId)
                                            .GroupBy(item => new {
                                                item.SubjectId,
            ...
            var currentQuery = queryable.Where(item => item.VoucherDate >= startDate && item.VoucherDate <= endDate )
                                            .SelectMany(item => item.Details)
                                            .WhereIf(subjectId != null, item => item.SubjectId == subjectId)
