...
$(function () {
    ...
    const getVoucher = () => {
        return {
            ...
            voucherDate: new Date(), creditorId: '', details: []
        }
    }
    const useVoucherStore = defineStore('voucher', () => {
        ...
        const setEditItem = (payload) => {
            ...
            editItem.value = { ...others, details: newDetails, creditorId };
    ...
    const useEditModalStore = defineStore('editModal', () => {
        ...
        const showModal = (payload) => {
            ...
            voucherStore.editItem.creditorId = ''
        }
  ...
  <ul class="pagination">
    ...
    <table class="table table-responsive table-striped align-middle">
                ...
                <tfoot>
                    ...
                    <td v-if="isDraftStatus" colspan="3" class="text-end">
    ...
    const ReceivableDetail = {
        ...
        setup(props, { emit }) {
            ...
            const generateDetails = () => {
                ...
                const creditorId = editItem.value.creditorId;
                if (!creditorId) {
                    abp.message.info(l('PleaseEnterCreditor'));
                    return
                }
                if (payments.length === 0) {
                    abp.message.info(l('PleaseEnterPaymentItemInfo'));
                    return
                }
                const param = {
                ...
                voucherRequests.generateDetails
                    (param).then(result => {
                        ...
                        setEditItem({ item: { ...teditItem.value, details: [] }, isClearData: false });
                    })
            }
...
<Modal v-if="isShow" :value="isShow" @input="input" @save="save" :title="l('Detail')" modal-id="edit-modal">
    ...
    const EditHeader = {
        ...
        setup(props, { emit }) {
            const companyStore = useCompanyStore();
            const { clients } = storeToRefs(companyStore);
            const { setCompanies } = companyStore;
            ...
            onMounted(() => {
                nextTick(() => { 
                    initCompanySelect(true, '#creditorId', '#creditor',
                        e => {
                            editItem.value.creditorId = e.params.data.id;
        ...
        <div class="items">
            <table class="table table-responsive table-striped" :style="tableStyle">
                <colgroup>
                    ...
                    <col style="width: 250px;" />
                    <col style="width: 120px;" />
                    <col style="width: 120px;" v-if="isDraftStatus" />
                </colgroup>
                <thead>
                <tr>
                    <th v-if="isDraftStatus">{{l('Actions')}}</th>
    ...
    :title="l((isDraftStatus ? editItem.id ? 'Edit' : 'New': 'View') + 'ReceivableVoucher')" :show-submit="isDraftStatus">
<div id="content">
    <EditHeader v-if="isShowHeader && (editItem.id && editItem.creditorId  || !editItem.id )" :errors="errors" @creditor-change="getDetailsByCreditor" />
    ...
    const EditModal = {
        ...
        data() {
            ...
            const getDetailsByCreditor = (newPage) =>{
                const params = {
                    creditorId: editItem.value.creditorId,
                    maxResultCount,
