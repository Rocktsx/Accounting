$(function () {
    ...
    function handleData(items) {
        ...
        items.reduce((prev, current) => {
            ...
            group.presentedBalance += current.presentedAmount * current.debitorCreditor

            current.presentedBalance = group.presentedBalance

            group.balance += current.nativeAmount * current.debitorCreditor;
    ...
    $(document).on('click', '#searchBtn', function () {
        ...
        accounting.finance.reports.bankReconciliationReport.getList(params).then(function (result) {
        ...
        <table class="table table-borderless">
            <thead>
                ...
                <tr class="border-bottom">
                    ...
                    <th class="fw-bold text-end">{{ l('BookBalance') }}</th>
                    <th class="fw-bold">P</th>
                    <th class="fw-bold text-end">{{ l('BankBalance') }}</th>
                </tr>
                <tr>
                    <th colspan="3"></th>
            ...
            <tbody>
                <template  v-for="item in items" :key="item.code">
                    ...
                    <tr v-for="subItem in item.items" :key="subItem.voucherCode">
                        ...
                        <td>{{ subItem.isPresented ? 'P': 'U' }}</td>
                        <td class="text-end">{{ renderAmount(Math.abs(subItem.presentedBalance)) }}</td>
                    </tr>
